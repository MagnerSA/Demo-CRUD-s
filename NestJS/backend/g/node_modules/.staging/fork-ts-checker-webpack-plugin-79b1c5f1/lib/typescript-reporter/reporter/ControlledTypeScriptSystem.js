...
function createControlledTypeScriptSystem(mode = 'readonly') {
    // watchers
    const fileWatchersMap = new Map();
    const directoryWatchersMap = new Map();
    const recursiveDirectoryWatchersMap = new Map();
    ...
    function invokeFileWatchers(path, event) {
        ...
        if (fileWatchers) {
            ...
            fileWatchers.forEach((fileWatcher) => fileWatcher(forwardSlash_1.default(normalizedPath), event));
        }
    }
    function invokeDirectoryWatchers(path) {
        ...
        if (directoryWatchers) {
            directoryWatchers.forEach((directoryWatcher) => directoryWatcher(forwardSlash_1.default(normalizedPath)));
        }
        recursiveDirectoryWatchersMap.forEach((recursiveDirectoryWatchers, watchedDirectory) => {
            if (watchedDirectory === directory ||
                ...
                recursiveDirectoryWatchers.forEach((recursiveDirectoryWatcher) => recursiveDirectoryWatcher(forwardSlash_1.default(normalizedPath)));
            }
        });
    }
    ...
    const controlledSystem = Object.assign(Object.assign({}, ts.sys), { useCaseSensitiveFileNames: caseSensitive, fileExists(path) {
        ...
        watchDirectory(path, callback, recursive = false) {
            return createWatcher(recursive ? recursiveDirectoryWatchersMap : directoryWatchersMap, path, callback);
        }, 
